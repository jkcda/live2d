 ...
 *
 * 上限受两个条件限制，取更紧的那个：
 *   1. **素材**：虹膜左右各约 14px 眼白、上下约 8px，越过就把补出来的眼白推出眼睛轮廓；
 *   2. **观感**：这张图的虹膜几乎占满眼睛开口（72×44 的虹膜 vs ~100px 的眼宽），
 *      位移一大就变成"整只眼睛挪了"，而不是"瞳孔在眼白里滑" —— 实测 10px 就是这个效果，
 *      所以收到 5px（桌宠窗口约 1.2px）。真实眼睛本来就是整块虹膜在转，
 *      瞳孔不能单独跑；这张图的虹膜太大，所以只能靠小幅度 + 整体视差来表现。
 */
const PUPIL_SHIFT_MAX_PX = 5
